...
import designDecision from "./react-error-boundary-design-decision.png";
import workaround from "./react-error-boundary-workaround.png";

export const CaptionedImage = ({ src, alt, caption }) => (
	<figure>
		<Image
			src={src}
			alt={alt}
			style="border-radius: 0.5rem; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);"
		/>
...
<CaptionedImage
	src={designDecision}
	alt="Design decision explanation from React team"
	caption="Why are Error Boundaries not triggered for event handlers (Source: github.com/facebook/react/issues/11409#issuecomment-340859253)"
/>
...
<CaptionedImage
	src={workaround}
	alt="Workaround for handling async errors in React components"
	caption="Why are Error Boundaries not triggered for event handlers (Source: github.com/facebook/react/issues/11409#issuecomment-783309449)"
/>
